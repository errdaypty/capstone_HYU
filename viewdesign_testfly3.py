    ...
    def run(self):
        ...
        for i in self.frameList:
            ...
            if ret:
                ...
                opening = cv2.adaptiveThreshold(grayimg, 255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,55,5)
...
class MyWindow(QMainWindow, form_class):
    ...
    def __init__(self, parent=None):
        ...
        self.enablepcs_PB.toggled.connect(self.toggled_ImageProcessStart)


        #### for windrose

        # list for storing cnts
        self.a = []
    ...
    def storeFrames(self,frames):
        ...
        print('frame정체',frames)
        print('b 길이',len(self.b))
            ...
            if len(self.coordination_array) >= 2:
                ...
                for i in range(limit):
                    for j in range(limit):
                        ...
                        #    continu
                        #else:
                        a[i, j] = math.sqrt((self.coordination_array[nowframe - self.initation[0]-1][i][0] -self.coordination_array[nowframe - self.initation[0]][j][0]) ** 2
                                            + (self.coordination_array[nowframe - self.initation[0]-1][i][1] -self.coordination_array[nowframe - self.initation[0]][j][1]) ** 2)
