...
# return cap
    ...
    def returntemporaryframe(self,kkk):
        cap = cv2.VideoCapture('커피_무반응.mp4')
    ...
    # for i in range(nr_of_frames):
        ...
        for i in self.frameList:
            ...
            opening = cv2.adaptiveThreshold(grayimg, 255,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY_INV,15,5)
            ...
            # 현재 프레임을 반환하는 새로운 시그널을 만들어서 보내보자
            # self.videoSlider.setValue(int(cap.get(cv2.CAP_PROP_POS_FRAMES)))
...
class MyWindow(QMainWindow, form_class):
    ...
    def __init__(self, parent=None):
        ...
        self.enablepcs_PB.clicked.connect(self.imageProcessStart)


        #### 얘를.. 선언 해 줘야되나??? 안해도 될 거 같다
        self.a = []
    ...
    def play_Video(self,state):
        ...
        #self.play_PB.setPushButtonStyle(QtCore.Qt.PushButtonTextUnderIcon)
        #else:
    ...
    def pause_Video(self):
        stop = False
        self.video_Pause_Signal.emit(stop)
    ...
    def storeContours(self,cnts):
        self.a.append(cnts)
        print('a length',len(self.a))


    def imageProcessStart(self,nr_of_frames):
        self.fig = plt.figure(0)
        ax = self.fig.add_subplot(111)

        self.canvas = FigureCanvas(self.fig)
        #모니터 좌 상단으로부터 가로 50, 세로 450, 크기 가로 250, 세로 150px
        ...
        origin = [0], [0]
        Q = ax2.quiver(*origin, [], [], color='m', scale=1000)
    ...
    def scatter_ImageProcess(self, cnts):
        ...
        for a, b in enumerate(cnts):
            ...
            if cv2.contourArea(b) < 150 and M["m00"]!=0:
                cX = int(M["m10"] / M["m00"])
                cY = int(M["m01"] / M["m00"])
